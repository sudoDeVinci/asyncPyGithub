...
async def get_repo_languages(
    ...
    return (
        ...
        cast(dict[str, int], response.json())
    )
...
async def get_repos_authenticated(
    ...
    }) if type is None else params.update({
        'type': type,
    })
...
async def get_repos_authenticated_extended(
    ...
    outputSlice: RepoSlice = RepoSlice(
        count=0,
        repos = [],
        errors=[]
    )
    ...
    if reporesponse[0] != 200:
        LOGGER.error(f"Failed to retrieve repositories with::{reporesponse[0]}::{reporesponse[1]}")
    ...
    LOGGER.info(f"get_repos_authenticated_extended:::Retrieved {outputSlice['count']} repositories for authenticated user.")
    return outputSlice
